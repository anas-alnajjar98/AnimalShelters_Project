...
export class AnimalComponent implements OnInit {
  ...
  filteredAnimals: any[] = [];
  breedFilter: string = '';
  minAgeFilter: number = 0;  // Initialize minAge
  maxAgeFilter: number = 20; // Initialize maxAge
  ...
  matchAge(animalAge: number): boolean {
    return animalAge >= this.minAgeFilter && animalAge <= this.maxAgeFilter;
  }

  onMinAgeChange(): void {
    if (this.minAgeFilter > this.maxAgeFilter) {
      this.maxAgeFilter = this.minAgeFilter; // Sync max age to min age
    }
    this.applyFilters();
  }
  ...
  onMaxAgeChange(): void {
    if (this.maxAgeFilter < this.minAgeFilter) {
      this.minAgeFilter = this.maxAgeFilter; // Sync min age to max age
    }
    this.applyFilters();
  }
}
