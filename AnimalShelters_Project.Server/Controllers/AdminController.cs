...
namespace AnimalShelters_Project.Server.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<IActionResult> AcceptApplication(int applicationId)
        {
            ...
            var applicantName = application.User.UserName;
            var animalName = animal.Name;

            await SendEmailAsync(applicantEmail, "Adoption Application Accepted",
                $"Dear {applicantName},<br><br>Your adoption application for {animalName} has been accepted. " +
                "You may come to our center to complete the adoption process. We look forward to seeing you!<br><br>Best regards,<br>Your Adoption Center");

            // Send rejection emails to other applicants
            foreach (var otherApp in otherApplications)
            {
                var otherApplicantEmail = otherApp.User.Email;
                var otherApplicantName = otherApp.User.UserName;
                await SendEmailAsync(otherApplicantEmail, "Adoption Application Rejected",
                    $"Dear {otherApplicantName},<br><br>We regret to inform you that your adoption application for {animalName} was not successful. " +
                    "We hope you will consider adopting another pet with us. Thank you for your interest and understanding.<br><br>Best regards,<br>Your Adoption Center");
            }
        ...
        private async Task SendEmailAsync(string toEmail, string subject, string body)
        {
            ...
            using var client = new SmtpClient(mailSettings["SmtpHost"])
            {
                Port = int.Parse(mailSettings["SmtpPort"]),
                Credentials = new NetworkCredential(mailSettings["SmtpUser"], mailSettings["SmtpPass"]),
                EnableSsl = true
            };

            var mailMessage = new MailMessage
            {
                From = new MailAddress(mailSettings["FromEmail"], mailSettings["DisplayName"]),
                Subject = subject,
                Body = body,
                IsBodyHtml = true
            };
